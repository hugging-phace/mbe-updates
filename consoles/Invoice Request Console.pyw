...
def process_queue():
    ...
    word_fade_index = [0]
    THEME_PURPLE = "#9b59b6"
    THEME_PURPLE_HOVER = "#8e44ad"
    LIGHTER_PURPLE = "#b39ddb"
    DEFAULT_WHITE = "#ffffff"

    def start_text_fade(on_complete_callback):
        """Fade text out into background color cleanly, then fire callback."""
        fade_state[0] = 0
        
        def animate():
            fade_state[0] += 1
            if fade_state[0] <= 10:
                r = int(255 - (fade_state[0] * (255 - 42) / 10))
                g = int(255 - (fade_state[0] * (255 - 42) / 10))
                b = int(255 - (fade_state[0] * (255 - 42) / 10))
                
                hex_color = f"#{r:02x}{g:02x}{b:02x}"
                reason_text.configure(text_color=hex_color)
                fade_job[0] = root.after(40, animate)
            else:
    ...
    def polish_text_thread():
        ...
        if len(words) <= 4:
            polished = current_text.strip()
            if polished:
                polished = polished[0].upper() + polished[1:]
            start_text_fade(lambda: update_reason(polished))
            return
...
{current_text}"""
    ...
    def update_reason(polished):
        """Clears text box and inputs polished text back using a standard purple typing effect."""
        stop_text_fade()
        
        reason_text.delete("1.0", "end")
        reason_text.configure(text_color=LIGHTER_PURPLE)
        
        words = polished.strip().split()
        word_fade_words[0] = words
        word_fade_index[0] = 0
        
        def fade_in_words():
            if word_fade_index[0] < len(word_fade_words[0]):
                current_text = reason_text.get("1.0", "end").strip()
                if current_text:
                    new_text = current_text + " " + word_fade_words[0][word_fade_index[0]]
                else:
                    new_text = word_fade_words[0][word_fade_index[0]]
                
                reason_text.delete("1.0", "end")
                reason_text.insert("1.0", new_text)
                word_fade_index[0] += 1
